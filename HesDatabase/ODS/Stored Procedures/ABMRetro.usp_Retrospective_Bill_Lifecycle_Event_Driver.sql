...
CREATE OR ALTER PROCEDURE [ABMRetro].[usp_Retrospective_Bill_Lifecycle_Event_Driver]
	@Topic VARCHAR(255),	-- Must be a parameter because topic can change per environment
	@Debug BIT = 0
AS 

/*****************************************************
* Name:		ABMRetro.usp_Retrospective_Bill_Lifecycle_Event_Driver
* Purpose:  BIO-1277
...
* Modified By	: Anna Gekhtman
* Purpose		: BIO-1806 - Added RetroBillHeldEvent and RetroBillReleasedEvent
* Modified Date	: 7/10/2025
* Modified By	: Franz Megie
* Purpose		: BIO-1446 - RetroBillResentEvent
* Modified Date	: 10/22/2025
* Modified By	: Anna Gekhtman
* Purpose		: BIO-2281, BIO-2283 - Resend Bill changes
	...
	BEGIN TRY
		...
		FROM		(
						...
						AND			Message_Type IN (
										...
										'RetroBillCreatedEvent',
										'RetroBillRejectedEvent',
										'RetroBillPhaseChangedEvent',
										'RetroBillAllocatedEvent',
										'RetroBillAcceptedEvent',
										'RetroBillRefusedEvent',
										'RetroBillConvertedEvent',
										'RetroBillCompletedEvent',
										'RetroBillHeldEvent',
										'RetroBillReleasedEvent'
									)
						UNION
						
						-- A delay is added for RetroBillResentEvent to allow the RetroBillAllocated stored proc to process a Resent event that may have come in immediately before the Allocated event
						-- Any Resent events still remaining after an hour can be processed by the standard RetroBillResentEvent proc
						SELECT		Retro_Event_Log_Id,
									Message_Type,
									Topic_Partition_Num,
									Topic_Offset_Num
						...
						WHERE		Process_Status = 'Loaded'
						AND			Topic = @Topic
						AND			Message_Type = 'RetroBillResentEvent'
						AND			ETL_Date_Time_Created < DATEADD(HOUR,-1,GETDATE()) --ResentEvent that came more than 1 hour ago 
					) l
		ORDER BY	l.Topic_Partition_Num ASC,
					l.Topic_Offset_Num ASC
		...
		BEGIN
			...
			-- Note: All procs MUST be named "ABMRetro.usp_Retrospective_Bill_Lifecycle_" followed by the message type handled by that proc
			SELECT @SQL =
				CASE
					WHEN @Duplicate_Flag = 1 THEN '-- Duplicate detected for Retro_Event_Log_Id ' + Convert(varchar(20), @Current_Retro_Event_Log_Id)
					ELSE 'EXECUTE ABMRetro.usp_Retrospective_Bill_Lifecycle_' + @Current_Message_Type + ' @Retro_Event_Log_Id = ' + Convert(varchar(20), @Current_Retro_Event_Log_Id)
				END
