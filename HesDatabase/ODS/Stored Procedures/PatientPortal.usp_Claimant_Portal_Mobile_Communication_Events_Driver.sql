...
CREATE OR ALTER PROCEDURE [PatientPortal].[usp_Claimant_Portal_Mobile_Communication_Events_Driver]
	@Topic VARCHAR(255),	-- Must be a parameter because topic can change per environment
	@Debug BIT = 0
AS 

/*****************************************************
* Name:		PatientPortal.usp_Claimant_Portal_Mobile_Communication_Events_Driver
* Purpose:  BIO-1524
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			-- Note: All procs MUST be named "PatientPortal.usp_Claimant_Portal_Mobile_Communication_Events_" followed by the message type handled by that proc
			SELECT @SQL =
				CASE
					WHEN @Duplicate_Flag = 1 THEN '-- Duplicate detected for Mobile_Event_Log_Id ' + Convert(varchar(20), @Current_Mobile_Event_Log_Id)
					ELSE 'EXECUTE PatientPortal.usp_Claimant_Portal_Mobile_Communication_Events_' + @Current_Message_Type + ' @Mobile_Event_Log_Id = ' + Convert(varchar(20), @Current_Mobile_Event_Log_Id)
				END
